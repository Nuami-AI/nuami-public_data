...
/** 리포지토리 내 기본 통계연보 엑셀 경로 (Vercel 배포용) */
export const DEFAULT_YEARBOOK_DIR = path.join(process.cwd(), "data", "yearbook");

/** 통계연보 엑셀 디렉터리 — env 미설정 시 data/yearbook 사용 */
export function getYearbookDir(): string | null {
  const configured = process.env.YEARBOOK_XLSX_DIR?.trim();
  const candidates = configured ? [configured, DEFAULT_YEARBOOK_DIR] : [DEFAULT_YEARBOOK_DIR];
